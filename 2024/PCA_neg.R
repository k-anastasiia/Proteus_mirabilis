...
setwd('~/Desktop/Proteus_mirabilis_2024/Negative_mode/Multivariate')
getwd()

metadata<- read_csv("Neg_Proteus_mirabilis_2024_metadata_no_QC.csv")
metadata <- metadata[-1:-6,] #exclude blank
metadata <- metadata[-12,] #exclude
scaled_table <- read_csv("CLR_Scaled_neg.csv")
...
#pbt vs wt
metadata_b3_dv <- metadata_b3[-9:-12,]
metadata_b3_dv <- metadata_b3_dv[-1:-4,]
data_merge_b3_dv <- metadata_b3_dv %>% dplyr::select("filename", "ATTRIBUTE_group") %>% 
  left_join(scaled_table) %>% 
  column_to_rownames("filename")

res.pca_d2 <- data_merge_b3_dv %>% dplyr::select(-ATTRIBUTE_group) %>% prcomp(scale = FALSE) #TRUE if scaling was not performed before
fviz_eig(res.pca_d2)
fviz_pca_ind(res.pca_d2, col.ind = data_merge_b3_dv$ATTRIBUTE_group, addEllipses = TRUE, repel = TRUE, 
             legend.title = "Group", palette = c("red3", "royalblue"),
             geom.ind = "point", pointsize = 2)
