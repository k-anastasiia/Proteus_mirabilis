...
setwd('~/Desktop/proteus_mirabilis/neg')
getwd()
#upload feature table and metadata
md <- read.csv('Neg_Proteus_mirabilis_2024_metadata_no_QC.csv', header = T, check.names = F)
ft <- read.csv('Negative_no_QC_iimn_gnps_quant.csv', header = T, check.names = F, sep = ',')
...
write.csv(new_md, "md_new.csv",row.names = TRUE)
...
#scaling
#Imputed <- read_csv("~/Desktop/GitHub/Data_cleanup/Imputed_QuantTable_filled_with_126_CutOff_Used_0.3.csv")
...
write.csv(Imp_clrt, "CLR_Scaled_neg.csv",row.names = TRUE)
